...
def pca(ms_lr_orig):
    ...
    _, B, H, W = ms_lr_orig.shape
    ms_lr = torch.clone(ms_lr_orig).double()
    ...
    return pca_image.float(), U.float(), mean.float()


def inverse_pca(pca_image, pca_matrix, mu):
    """
    ...
    flattened_image = torch.matmul(-pca_matrix, flattened_pca) + mu
